...
final class ViewTeamTest extends FunctionalTestCase
{
    ...
    public function testEveryTeamMemberIsListedAsReviewer(): void
    {
        ...
        foreach (array_values($this->getService(MemberRepository::class)->findAll()) as $i => $member) {
            $this->assertSelectorTextContains(sprintf('[data-test-reviewers] ul > li:nth-child(%d) > p', $i + 1), $member->getFullname());
        }
    }
    ...
    public function testEveryTeamMemberComments(): void
    {
        ...
        $commenters = $crawler->filter('[data-test-review-comment] > div > div > p')->each(static fn (Crawler $a): string => $a->text());
